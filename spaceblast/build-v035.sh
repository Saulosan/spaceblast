#!/bin/sh
# build-v035.sh — compila a ROM final da apresentacao/textos v0.35.
...
PATH="$BUILD_TMP:$PATH" cvbasic --nes space-blast-v035.bas space-blast-v035.asm
...
sed -i 's/space-blast-v035\.asm $/space-blast-v035.asm/' space-blast-v035.asm
sed -i 's/^\t; Created: .*/\t; Created: reproducible build/' space-blast-v035.asm
...
sed -i 's/LDA #\$1e\t; Color normal, Sprites visible, Background visible, No clipping, Color./LDA #$00\t; v0.16: boot sem flash cinza (video so liga no SCREEN ENABLE)/' space-blast-v035.asm
...
sed -i '0,/\tJSR copy_chrram/s//\tLDA #$3F\n\tSTA PPUADDR\n\tLDA #$00\n\tSTA PPUADDR\n\tLDA #$0F\n\tSTA PPUDATA\n\tJSR copy_chrram/' space-blast-v035.asm
...
sed -i 's/^PPUSIZE:\tEQU \$40$/PPUSIZE:\tEQU $90\t; v0.20 burst stream/' space-blast-v035.asm
PATH="$BUILD_TMP:$PATH" gasm80 space-blast-v035.asm -o space-blast-v035.nes
